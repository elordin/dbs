CREATE FUNCTION electionResults(INT)
RETURNS TABLE (lastname VARCHAR(255), firstname VARCHAR(255), pid INT, partyname VARCHAR(255), fsid INT, federalstatename VARCHAR(255), rankInFS BIGINT)
AS $results$
...
PartiesBeyondFivePercent2(pid, votes) AS (
    ...
    WHERE a1.year = $1
       AND a1.votes > 0
       AND (p.isminority
        OR a1.votes >= 0.05 * (SELECT SUM(votes) from AggregatedZweitstimmenForParty  where year = $1)
...
SeatsPerLandesliste (llid, seats)  AS(
    ...
    WHERE ll.year = $1 AND ll.pid in (select pid from PartiesBeyondFivePercent) AND (coalesce(wkspll.seats,0)+coalesce(astdkpll.seats,0)) >0
),
...
--unions the Direktkandidaturen with the Listenplätzen
 MergedCandidates (idno, fsid, pid, wkid, rank) AS (
    ...
    (select c.idno, ll.fsid, ll.pid, NULL as wkid,llp.position as rank
     ...
     NATURAL JOIN Landesliste ll
     WHERE ll.year = $1 AND ll.pid in (select pid from PartiesBeyondFivePercent)
...
---------------- STEP 5: List of Delegates for Bundestag ----------------
SELECT  c.Lastname, c.Firstname, p.pid, p.name AS partyname, fs.fsid, fs.name AS federalstatename, d.rankInFS
FROM Delegates d
NATURAL JOIN Candidates c
NATURAL JOIN Federalstate fs
JOIN Party p ON p.pid=d.pid
ORDER BY p.pid, fs.fsid
-- WHERE p.name='CDU'
