...
Delegates (idno, fsid, pid, wkid, rankInFS) AS (
    ...
    LEFT OUTER JOIN SeatsPerLandesliste spll ON ll.llid = spll.llid
...
select count(*) -- c.Lastname, c.Firstname, p.name, fs.name, d.rankInFS
from Delegates d
NATURAL JOIN Candidates c
NATURAL JOIN Federalstate fs
LEFT OUTER JOIN Party p ON p.pid=d.pid
