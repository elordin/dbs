...
WITH RECUSIVE Sitzverteilung(totalseats, ausgleichmissing, pid, mandate) AS (
    WITH RECURSIVE
    ...
    MandateMitUerberhang AS (
        SELECT  SUM(rd.mandate) + SUM(um.uhms)

                rd.pid, rd.mandate + COALESCE(uhms, 0) AS Mandate
    ...
    UNION ALL
        ...
        WHERE sv.ausgleichmissing > 0
SELECT pid, mandate
FROM Sitzverteilung
WHERE ausgleichmissing = 0
