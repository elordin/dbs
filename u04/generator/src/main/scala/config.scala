...
object GeneratorConfig {
    ...
    def allWKConfigs(year:Int):Map[Int, GeneratorConfigFromDatabase] = {
        ...
        wkids.map((wkid:Int) => (wkid, new GeneratorConfigFromDatabase("localhost", 5432, "wisdb", "postgres", "abc123", year, wkid))).toMap
    }

    val filename:String = "insert_stimmzettel.sql"

    case class GeneratorConfigFromDatabase(host:String, port:Int, dbname:String, user:String, password:String, year:Int, wkid:Int) {
        ...
        def existingCitizens:List[String] = {
            if (_existingCitizens == null) {
                _existingCitizens = withDatabase(host, port, dbname, user, password, conn => {
                    val statement = conn.createStatement()
                    val resultset:ResultSet = statement.executeQuery(
